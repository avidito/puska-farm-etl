-- public.pencatatan_ternak_masuk_cdc source

CREATE OR REPLACE VIEW public.pencatatan_ternak_masuk_cdc
AS WITH cte_raw_parse AS (
         SELECT log_cdc_prc.operation,
            ...
            ((log_cdc_prc.old_data ->> 'jenis_mitra_pengirim'::text))::character varying(255) AS old_jenis_mitra_pengirim,
            ...
            ((log_cdc_prc.new_data ->> 'jenis_mitra_pengirim'::text))::character varying(255) AS new_jenis_mitra_pengirim,
           ...
           FROM log_cdc_prc
          WHERE log_cdc_prc.table_name::text = 'pencatatan_ternak_masuk'::text AND log_cdc_prc.is_processed IS FALSE
        ), cte_op_insert AS (
