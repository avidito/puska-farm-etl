-- public.distribusi_susu_cdc source
CREATE OR REPLACE VIEW public.distribusi_susu_cdc
AS
WITH cte_raw_parse AS (
  SELECT
    log_cdc_prc.operation,
    log_cdc_prc."timestamp",
    (log_cdc_prc.old_data ->> 'id'::text)::bigint AS old_id,
    (log_cdc_prc.old_data ->> 'tgl_distribusi'::text)::date AS old_tgl_distribusi,
    ...
    (log_cdc_prc.old_data ->> 'id_mitra_bisnis'::text)::bigint AS old_id_mitra_bisnis,
    ...
    (log_cdc_prc.new_data ->> 'tgl_distribusi'::text)::date AS new_tgl_distribusi,
    ...
    (log_cdc_prc.new_data ->> 'id_mitra_bisnis'::text)::bigint AS new_id_mitra_bisnis,
    ...
    FROM log_cdc_prc
  WHERE log_cdc_prc.table_name::text = 'distribusi_susu'::text
    AND log_cdc_prc.is_processed IS FALSE
), cte_op_insert AS (
  SELECT
    cte_raw_parse.new_id AS id,
    cte_raw_parse.new_tgl_distribusi AS tgl_distribusi,
    cte_raw_parse.new_id_unit_ternak AS id_unit_ternak,
    cte_raw_parse.new_id_jenis_produk AS id_jenis_produk,
    cte_raw_parse.new_id_mitra_bisnis AS id_mitra_bisnis,
  ...
  SELECT new_1.id,
    new_1.tgl_distribusi,
    new_1.id_unit_ternak,
    new_1.id_jenis_produk,
    new_1.id_mitra_bisnis,
    new_1.satuan,
    ...
    FROM ( SELECT cte_op_update_flag.old_id AS id,
            cte_op_update_flag.old_tgl_distribusi AS tgl_distribusi,
            cte_op_update_flag.old_id_unit_ternak AS id_unit_ternak,
            cte_op_update_flag.old_id_jenis_produk AS id_jenis_produk,
            cte_op_update_flag.old_id_mitra_bisnis AS id_mitra_bisnis,
      ...
      JOIN ( SELECT cte_op_update_flag.new_id AS id,
            cte_op_update_flag.new_tgl_distribusi AS tgl_distribusi,
            cte_op_update_flag.new_id_unit_ternak AS id_unit_ternak,
            cte_op_update_flag.new_id_jenis_produk AS id_jenis_produk,
            cte_op_update_flag.new_id_mitra_bisnis AS id_mitra_bisnis,
  ...
  SELECT cte_raw_parse.old_id AS id,
    cte_raw_parse.old_tgl_distribusi AS tgl_distribusi,
    cte_raw_parse.old_id_unit_ternak AS id_unit_ternak,
    cte_raw_parse.old_id_jenis_produk AS id_jenis_produk,
    cte_raw_parse.old_id_mitra_bisnis AS id_mitra_bisnis,
 ...
 SELECT cte_cdc_view.id,
    cte_cdc_view.tgl_distribusi,
    cte_cdc_view.id_unit_ternak,
    cte_cdc_view.id_jenis_produk,
    cte_cdc_view.id_mitra_bisnis,
    cte_cdc_view.satuan,
    cte_cdc_view.jumlah,
