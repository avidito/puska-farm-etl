-- public.produksi_susu_cdc source

CREATE OR REPLACE VIEW public.produksi_susu_cdc
AS WITH cte_raw_parse AS (
         SELECT log_cdc_prc.operation,
           ...
           FROM log_cdc_prc
          WHERE log_cdc_prc.table_name::text = 'produksi_susu'::text AND log_cdc_prc.is_processed IS FALSE
        ), cte_op_insert AS (
